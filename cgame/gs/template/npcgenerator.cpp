...
//			}
			...
			{
				nt.npc_data.service_association_enter = 1;
			}
						...
						int ai_policy_cid,int __aggro_policy,gnpc * origin_npc, int life)
{
	...
	/*166+ Reino do Norte*/
	if(pTemplate->npc_data.service_association_enter)
	{
...
npc_generator::LoadGenData(world* plane, CNPCGenMan & npcgen, rect & world_region)
{
	...
	for(unsigned int i = 0; i <count; i ++)
	{
		...
		for(unsigned int j = (unsigned int)area.iFirstRes; j < (unsigned int)area.iFirstRes+ area.iResNum; j ++)
		{
			...
			GLog::log(GLOG_ERR,"NPCGENMAN:: Load ResArea res.fHeiOff = %f",res.fHeiOff);

			float fHeight = _plane->GetHeightAt(area.vPos[0],area.vPos[2]);

			GLog::log(GLOG_ERR,"NPCGENMAN:: Load ResArea fHeight = %f",fHeight);

			pSp->SetOffsetTerrain(res.fHeiOff - fHeight);
			pSp->AddEntry(ent);
			entry_count ++;
		}
