...
class IA_EA_ESS_NEW	 : public essence_addon
{
public:
	virtual int GenerateParam(int datatype, addon_data & data,int arg_num)
	{
		ASSERT(datatype == (int)DT_NEW_ARMOR_ESSENCE);
		if(datatype != (int)DT_NEW_ARMOR_ESSENCE) return -1;
	...
	{
		ASSERT(data.arg[0] > 0);
		new_armor_essence * ess = (new_armor_essence*)essence;
...
template <unsigned int offset>
class IA_EA_ESS_NEW_SCALE : public essence_addon
{
public:
	virtual int GenerateParam(int datatype, addon_data & data,int arg_num)
	{
