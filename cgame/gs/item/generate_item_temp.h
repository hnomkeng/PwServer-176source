...
int generate_new_armor(unsigned int id, ID_SPACE idspace, char ** data, unsigned int& size, RAND_CLASS cls,element_data::GEN_ADDON_MODE normal_addon,const void * tag,unsigned int tag_size,int * sa_list = NULL)
{
	...
	size = sizeof(item_data) + sizeof(_item_content) + sizeof(_new_armor_essence);
	unsigned int hole_num = 0;
	if(normal_addon == element_data::ADDON_LIST_DROP)
	{
		hole_num = element_data::RandSelect(&(ess->drop_probability_socket0), sizeof(float), 5, cls,element_data::LOWER_TREND);		//�׶�����Ŀ
	...
	*(int*)buf = addon_num;		buf += sizeof(int);
	...
	for (int i = 0; i < addon_num; i++)
	{
		...
		*(int *)buf = addon_list.id;
		...
		/*int addons_speed[] = { 3448, 3470, 3492, 0 };

		DATA_TYPE datatype;
		EQUIPMENT_ADDON * addon = (EQUIPMENT_ADDON *)get_data_ptr(addon_id, ID_SPACE_ADDON, datatype);
		if(addon == NULL || datatype != DT_EQUIPMENT_ADDON)	return -1;

		bool res = false;

		for (int i = 0; i < sizeof(addons_speed) / sizeof(int); i++)
		{
			if (addon_id == addons_speed[i])
			{
				float value = 0.0f;

				if (addon_id == 3448)
					value = 0.1f + (std::rand() % 2) * 0.1f;
				else if (addon_id == 3470)
					value = 0.1f + (std::rand() % 4) * 0.1f;
				else if (addon_id == 3492)
					value = 0.2f + (std::rand() % 6) * 0.1f;

				float random_float = value;
				uint32_t getint;
...
int generate_camp_token_item(unsigned int id, ID_SPACE idspace, char ** data, unsigned int& size, RAND_CLASS cls)
{
	...
	*(int*)buf = DT_CAMP_TOKEN_ESSENCE;	buf += sizeof(unsigned int); //número da folha	
