...
unsigned int elf_item::InsertExp(unsigned int exp, short exp_level, gactive_imp* imp, bool& is_levelup, bool ischeck)//exp_level为经验丸（100）或人物等级
{
	...
	if(player_level > 105) player_level = 105;
...
bool elf_item::LevelUp(gactive_imp* imp)
{
	...
	X_EQUIP x_item;
	x_item.type = _tid; x_item.mask = GetIdModify();
	pImp->CalcEquipmentInfo();
	pImp->_runner->equipment_info_changed(1ULL<<item::EQUIP_INDEX_ELF,0,&x_item,sizeof(X_EQUIP));//此函数使用了CalcEquipmentInfo的结果
	return true;	
}
...
bool elf_item::ChangeElfSecureStatus(int status, bool ischeck)//状态转化
{
	...
	case STATUS_SECURE:
		if(status == STATUS_TRANSFORM)
		{
			...
			ess.status_value = g_timer.get_systime() + LuaManager::GetInstance()->GetConfig()->ChangeElfStatusTime;
