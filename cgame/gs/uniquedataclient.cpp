...
void UniqueDataClient::SUniqueData::init(int worldtag)
{
	...
	for(;iter != iend; ++iter)
	{
		...
		if(elem.inittype && !elem.vtype ) // δ��ʼ������ֵ
		{
			elem.wlock = true;
			elem.wsync = true;
...
void UniqueDataClient::OnDataLoad(int key,int type,int ver,const void* p,unsigned int sz)
{
	...
	if(elem.vtype && key == 0) // ֻ��һ��key�����¼reload log
	{
...
void UniqueDataClient::OnDataModify(int worldtag,int key,int type, const void* nval, unsigned int nsz,const void* oval, unsigned int osz, int retcode, int version)
{
	...
	if(elem.vtype && elem.vtype != type) // ��������������delivery�ϲ�һ�£����ܳ�����2��gsͬʱ��key���в�ͬ���ͳ�ʼ��
	{
		elem.wsync = false;
		elem.wlock = false;
		//���log ��delivery�ϵĽ�����б���ͬ��
		...
		&& (init || elem.stubval == oldval) ) // �Ǳ����޸�
	{
		...
		if(retcode) // error
		{
			...
			if(retcode == ERR_UNIQUE_VERSION_FAIL && elem.timever >= elem.clientver) // ����timeout�޸���汾δ�ύ�ɹ�,�������ύ
			{
				elem.wsync = true;
		...
		else // ���Ժϲ�ͬ�����޸�
		{
			...
			if(!init && !elem.wlock )  // �Ƕ�ռд��ͬ�� ��� ͬ���ڼ��Ƿ����˱����޸�
			{
				switch(elem.vtype)
				{
					case UDT_INT:
...
int UniqueDataClient::GetValueKey(int key)
{
	UDC_LOCK();
	if(!_data.exist(key))
		return -1;

	return _data[key].syncval.type();
}
