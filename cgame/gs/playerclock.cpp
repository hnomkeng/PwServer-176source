...
int player_clock::GetWeekNum(int nowtime)
{
	...
	int weeknum = (t1.tm_mday - 1) / 7; // ����1�Ź��˼���
	int daygap = (8 - t0.tm_wday) % 7;	// 1�ž�����1�м���
	if((t1.tm_mday-1)%7 >= daygap) weeknum ++; // ��ǰʱ����˼�����1
...
{
	ASSERT(oldtime <= nowtime && "���Ӽ��㲻������ʱ�������ʱ��");

	int passtime = nowtime - oldtime;
	int period = 0;

	if(type == GPC_PER_MONTH_LOCAL || type == GPC_PER_MONTH_GLOBAL) // �µļ����ͳһ
	{
		int cost = GetNextUpdatetime(type, oldtime) - oldtime;
		while(passtime >= cost && period < 12) // ���1��
		{
			++period;
			oldtime += cost;
			passtime -= cost;
...
void player_clock::InitFromDB(archive & ar,int roleid)
{
	...
	{		
		try
		{
			...
			{
				GLog::log(GLOG_ERR,"�û�%dʱ��װ�������ݹ���",roleid);
				return;
			}
		...
		{
			GLog::log(GLOG_ERR,"�û�%dʱ��װ�س���",roleid);
			_update_time.clear();
			return;
		}
	}
