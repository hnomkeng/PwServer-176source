...
int SkillWrapper::StartSkill( SKILL::Data & skilldata, object_interface player, const A3DVECTOR& target_pos,const XID * target, int size, int & next_interval)
{
	...
	{
		//Teleport with target calculates target's body size
		A3DVECTOR tpos;
		float tbody;
...
int SkillWrapper::InstantSkill( SKILL::Data & skilldata, object_interface player, const XID * target, int size)
{
	...
	// NewArmor
	if(skill->GetEventFlag() == EVENT_NEW_ARMOR)
	{
		int level = GetLevel(skilldata.id,player.GetClass(), true);
		if(level<=0) level = 1;
		...
		player.SetCoolDown(skilldata.id+COOLINGID_BEGIN,skill->GetCoolingtime()* 1000);		
		return SKILL_SUCCESS;

	}


	if(skill->IsInstant() || skill->IsInstantRun())
	{
		int level = GetLevel(skilldata.id,player.GetClass(), true);
		if(level<=0)
		{
...
bool SkillWrapper::EventNewArmor(object_interface player, int worldtag )
{
	PlayerWrapper w_player(player,this,NULL);
	for( StorageMap::iterator it = map.begin(); it != map.end(); ++ it )
	{
		const SkillStub * stub = SkillStub::GetStub((*it).first);
		if( stub && stub->IsPassive() && stub->GetEventFlag()==EVENT_NEW_ARMOR)
		{
			SkillKeeper skill = Skill::Create((*it).first);
			if( skill )
			{
...
SkillWrapper::SkillWrapper()
{
	...
	memset( glyph_pray_distance, 0, sizeof(glyph_pray_distance) ); 
	old_glyph_pray_distance = 0;

	memset( glyph_ap_cost, 0, sizeof(glyph_ap_cost) );
	memset( glyph_casting_time, 0, sizeof(glyph_casting_time) );

	glyph_pos_rollback_tag = 0;
	glyph_pos_rollback_pos = A3DVECTOR(0.f,0.f,0.f);

	timer_addition_pet_summon = 0;
	
	// Nível de Genesis
	genesis_level_enabled = false;
...
void SkillWrapper::Load( archive & ar )
{
	...
	ar.pop_back( glyph_pray_distance, sizeof(glyph_pray_distance) );
	ar >> old_glyph_pray_distance;

	ar.pop_back( glyph_ap_cost, sizeof(glyph_ap_cost) );
	ar.pop_back( glyph_casting_time, sizeof(glyph_casting_time) );

	ar >> glyph_pos_rollback_tag;
	ar >> glyph_pos_rollback_pos;

	ar >> timer_addition_pet_summon;
...
void SkillWrapper::Store( archive & ar )
{
	...
	ar.push_back( glyph_pray_distance, sizeof(glyph_pray_distance) );
	ar << old_glyph_pray_distance;

	ar.push_back( glyph_ap_cost, sizeof(glyph_ap_cost) );
	ar.push_back( glyph_casting_time, sizeof(glyph_casting_time) );
...
void SkillWrapper::LoadDatabase( archive & ar )
{
	...
	for (ar >> size; size > 0; --size)
	{
		...
		ar >> data.ability;
		ar >> data.level;
		if(id) map[id] = data;	//20120419 It is found that there is id=0 in the skill list of a player in the foreign server
	}
...
int SkillWrapper::NpcStart(ID id,object_interface npc, int level, const XID * target, int size, int& next)
{
	...
	int time = -1;	
	if(skill->GetStub() && skill->GetStub()->GetStateSize() >= 2)
	{
		skill->NpcFirstRun();
		time = skill->GetStub()->GetState(0)->GetTime(NULL);
...
int SkillWrapper::ActivateSkill(object_interface player, ID id, int level, int type)
{
	...
	//printf("ActivateSkill: skill id =  %d, type = %d \n", id, type);
	if(skill->IsDurative())
	{
		...
		skill->SetLevel( level );
		skill->StateAttack();
		//printf("ActivateSkill Durative: skill id =  %d, type = %d \n", id, type);
	}
	else if (type)
	{
		//printf("ActivateSkill type if: skill id =  %d, type = %d \n", id, type);
		if (type == 1)
		{
			//printf("ActivateSkill type == 1: skill id =  %d, type = %d \n", id, type);
		as[1].id = id;
		as[1].level = level;
	...
	{
		//printf("ActivateSkill else: skill id =  %d, type = %d \n", id, type);
		as[0].id = id;
		as[0].level = level;
...
int SkillWrapper::ActivateReboundSkill(object_interface player, ID id, int level, int trigger_prob, int period)
{
	...
	if(!skill)
		return 0;
	if(skill->IsDurative())
		return 0;       //Continuous skills should use ActivateSkill
	//printf("ActivateReboundSkill: skill id =  %d, level = %d \n", id, level);
	as[2].id = id;
	as[2].level = level;
...
int SkillWrapper::DeactivateReboundSkill(object_interface player, ID id, int /*level*/, int /*trigger_prob*/ , int /*period*/)
{
	...
	if(skill->IsDurative())
	{
    	return 0;       //Continuous skills should use DeactivateSkill
	} 
	else if( as[2].id == id )
	{
		as[2].id = 0;
...
void SkillWrapper::Swap(SkillWrapper& sw)
{
	...
	{
		std::swap(glyph_pray_distance[i].glyph_pray_distante_val, sw.glyph_pray_distance[i].glyph_pray_distante_val);
		std::swap(glyph_pray_distance[i].skill_id, sw.glyph_pray_distance[i].skill_id);
	}
	std::swap(old_glyph_pray_distance, sw.old_glyph_pray_distance);

	for (unsigned int i = 0; i < 3; i++)
	{
		std::swap(glyph_ap_cost[i].glyph_ap_cost_val, sw.glyph_ap_cost[i].glyph_ap_cost_val);
		std::swap(glyph_ap_cost[i].skill_id, sw.glyph_ap_cost[i].skill_id);
	}

	for (unsigned int i = 0; i < 3; i++)
	{
		std::swap(glyph_casting_time[i].glyph_casting_time_val, sw.glyph_casting_time[i].glyph_casting_time_val);
		std::swap(glyph_casting_time[i].skill_id, sw.glyph_casting_time[i].skill_id);
	}

	std::swap(glyph_pos_rollback_tag, sw.glyph_pos_rollback_tag);
	std::swap(glyph_pos_rollback_pos, sw.glyph_pos_rollback_pos);

	std::swap(timer_addition_pet_summon, sw.timer_addition_pet_summon);

	/*for(unsigned int i = 0; i < 3; i ++)
	{
		for(unsigned int j = 0; j < 6; j ++)
		{
			std::swap(glyph_damage_plus[i].skill_id[j], sw.glyph_damage_plus[i].skill_id[j]);
		}
		std::swap(glyph_damage_plus[i].glyph_damage_plus, sw.glyph_damage_plus[i].glyph_damage_plus);
	}
	...
	for(unsigned int i = 0; i < 3; i ++)
	{
		for(unsigned int j = 0; j < 6; j ++)
		{
			std::swap(glyph_critical_plus[i].skill_id[j], sw.glyph_critical_plus[i].skill_id[j]);
		}
		std::swap(glyph_critical_plus[i].glyph_critical_plus, sw.glyph_critical_plus[i].glyph_critical_plus);
	}
	...
	for(unsigned int i = 0; i < 3; i ++)
	{
		for(unsigned int j = 0; j < 6; j ++)
		{
			std::swap(glyph_casting_time[i].skill_id[j], sw.glyph_casting_time[i].skill_id[j]);
		}
		std::swap(glyph_casting_time[i].glyph_casting_time, sw.glyph_casting_time[i].glyph_casting_time);
	}*/
...
{
	//Clears the effects of passive skills
	EventUnreset(player);
	...
	for(; it!=ite; )
	{
		it->second.overridden = 0;	//overridden flag reset
		
		const SkillStub * stub = SkillStub::GetStub(it->first);
		if(!stub || stub->rank < 20 || stub->rank > 32) //Normal skills unchanged
		{
			++it;
			continue;	
		}
		...
		{
			//The fairy skills that are not in the conversion table are deleted	
			map.erase(it++);		
		}
		else
		{
			//If it exists in the conversion table, it will be converted according to it
			PersistentData data;
	...
	//Calculation is overridden flag
	StorageMap::iterator is;
	...
	//Reactivates the effect of passive skills
	EventReset(player);
...
{
	//Clear some bufs that affect balance
...
int SkillWrapper::GetCooldownTimer(ID id, object_interface npc, int level)
{
	SkillKeeper skill = Skill::Create(id);
	if(!skill)
		return -1;
	...
	return (int)(skill->GetStub()->GetCoolingtime(skill));
}
...
bool SkillWrapper::IsInstantRun(ID id)
{
	const SkillStub * stub = SkillStub::GetStub(id);
	if(!stub || !stub->IsInstantRun())
		return false;
	return true;
}
...
int SkillWrapper::RuneInstantSkill( SKILL::Data & skilldata, object_interface player, const XID * target, int size, int level)
{
	...
	//The client requested to send this message first
...
int SkillWrapper::SetPositionrollback2(object_interface player, int second)
{
	player.AddFilter(new filter_Positionrollback2(player, second, player.GetWorldTag(), player.GetPos()));
	return 0;
}


int SkillWrapper::ActivateDynGenesisLevel(object_interface player, ID id, int arg, int level)
{		
	SetGenesisLevelEnabled(true);
	player.SendClientGenesisLevel(true);
	return 0;
}

int SkillWrapper::DeactivateDynGenesisLevel(object_interface player, ID id, int arg, int level)
{
	SetGenesisLevelEnabled(false);
	player.SendClientGenesisLevel(false);
	return 0;
}

int SkillWrapper::NewArmorActivateSkill(object_interface player, ID id, int level)
{
	SkillKeeper skill = Skill::Create(id);
	if(!skill)
		return 0;
	...
	// Passivas (Novas)
 	if(!skill->IsPassive() && skill->IsDurative() && skill->GetEventFlag() == EVENT_NEW_ARMOR)
	{
		PlayerWrapper w_player(player,this,skill,0,0);
		w_player.SetEnable(true);
		skill->SetVictim(&w_player);
	...
	// Passivas (Old)
	if(skill->IsPassive() && skill->GetEventFlag() == EVENT_NEW_ARMOR)
	{
		PlayerWrapper w_player(player,this,skill,0,0);
		w_player.SetSkill(skill);
		skill->SetPlayer(&w_player);	
...
int SkillWrapper::NewArmorDeactivateSkill(object_interface player, ID id, int level)
{
	SkillKeeper skill = Skill::Create(id);
	if(!skill)
		return 0;
	...
	// Passivas
 	if(skill->IsDurative() && skill->GetEventFlag() == EVENT_NEW_ARMOR)
	{
		PlayerWrapper w_player(player,this,skill,0,0);
		w_player.SetEnable(false);
		skill->SetVictim(&w_player);
