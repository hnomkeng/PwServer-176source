...
gplayer_imp::PlayerOpenPersonalMarket(unsigned int count, const char name[PLAYER_MARKET_MAX_NAME_LEN], int * entry_list)
{
	...
	//OFFLINE MARKET
	unsigned char offline_market = 0;
	LuaManager * lua = LuaManager::GetInstance();
	int offline_market_item = GetInvPosItemTable(0 , lua->GetConfig()->ITEM_OFFLINE_MARKET);
	if (offline_market_item != -1) offline_market = 0x80;
		
	packet_wrapper h1(1024);
	using namespace S2C;
	CMD::Make<CMD::self_open_market>::From(h1,count,offline_market);
	//都通过了
	//检查物品是否合乎正确
	...
	//检查摊位买卖总价限制
	if(_stall_info.stallcard_id != -1 && (buy_money > GetGold() || sell_money > 4000000000u)) return false;
	if(_stall_info.stallcard_id == -1 && (buy_money > GetGold() || sell_money > MONEY_CAPACITY_BASE-GetGold())) return false;
...
gplayer_imp::CheckMarketTradeRequest(player_stall::trade_request & req, unsigned int &need_money, unsigned int& money_to_yinpiao, gplayer_imp * pBuyerImp)
{
	...
	unsigned int m_total =  m - money_to_yinpiao*WANMEI_YINPIAO_PRICE + GetGold();
	if(m_total < GetGold() || m_total > gplayer_money::MAX_GOLD)
	{
		//应当取消交易
...
gplayer_imp::CheckMarketPurchaseRequire(player_stall::trade_request & req, unsigned int total_price, unsigned int& money_to_yinpiao, gplayer_imp* pTraderImp)
{
	...
	unsigned int m_total =  total_price - money_to_yinpiao*WANMEI_YINPIAO_PRICE + GetGold();
	if(m_total < GetGold() || m_total > gplayer_money::MAX_GOLD)
	{
		//金钱过多，无法放下
		_runner->error_message(S2C::ERR_BUYER_MONEY_REACH_UPPER_LIMIT);
...
gplayer_imp::CheckMarketTradeRequire(player_stall::trade_request & req, unsigned int need_money, gplayer_imp * pTraderImp)
{
	...
	if(_player_state != PLAYER_STATE_NORMAL) return false;
	if(OI_TestSafeLock()) return false;
	if(GetGold() < need_money) return false;	//资金不足
...
gplayer_imp::DoPlayerMarketTrade(player_stall::trade_request & req, gplayer_imp * pImp, unsigned int need_money, unsigned int money_to_yinpiao)
{
	...
	//修改交易双方的金钱数量
	//这里应该已经保证了钱数必然正确
	ASSERT(need_money-money_to_yinpiao*WANMEI_YINPIAO_PRICE + GetGold() <= gplayer_money::MAX_GOLD && need_money-money_to_yinpiao*WANMEI_YINPIAO_PRICE + GetGold() >= GetGold() );
	pImp->SpendGold(need_money,true);
	GainGold(need_money-money_to_yinpiao*WANMEI_YINPIAO_PRICE);

	_runner->get_player_money(GetGold(), pImp->GetSilver() );
...
gplayer_imp::DoPlayerMarketPurchase(player_stall::trade_request & req, gplayer_imp * pImp, unsigned int total_price, unsigned int money_to_yinpiao)
{
	...
	//修改交易双方的金钱数量
	ASSERT(total_price-money_to_yinpiao*WANMEI_YINPIAO_PRICE + pImp->GetGold() <= gplayer_money::MAX_GOLD && total_price-money_to_yinpiao*WANMEI_YINPIAO_PRICE + pImp->GetGold() >= pImp->GetGold() );
	SpendGold(total_price,true);
	pImp->GainGold(total_price - money_to_yinpiao*WANMEI_YINPIAO_PRICE);
	//商店支出银票
	if(req.yinpiao)
	{
