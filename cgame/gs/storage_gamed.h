#ifndef __GNET_STORAGE_GAMED_H
#define __GNET_STORAGE_GAMED_H
...
public:
    ...
    std::future<void> GetRankingData(int type, int& count_ranking, std::vector<StorageGamed::DB_RANKING>& rankingData);
    ...
    std::future<void> SetArenaTeamInfo(const StorageGamed::DB_ARENA_TEAM& arenaTeam);
    ...
    std::future<void> GetArenaRankingData(int &teamCount, std::vector<StorageGamed::DB_TEAM_INFO> &teamData);
    std::future<void> ResetDailyArenaPoints();
    std::future<void> ResetWeeklyArenaPoints();
    std::future<void> AddOrUpdateArenaPlayer(const StorageGamed::DB_ARENA_PLAYER &player);
	...
	static StorageGamed * GetInstance()
	{
		if (!instance)
		instance = new StorageGamed();
		return instance;
	}
	static StorageGamed * instance;
};
