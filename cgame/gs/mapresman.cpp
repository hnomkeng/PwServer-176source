	...
	if(_mapres_type == MAPRES_TYPE_ORIGIN || _mapres_type == MAPRES_TYPE_SOLO_CHALLENGE)
	{
		...
		{	
			...
			for(unsigned int i = 0; i < roomnum; i++)
			{
				...
				std::string roomoffset = conf->find(section, format).c_str();
				sscanf(roomoffset.c_str(),"{%f,%f,%f}",&pos.x,&pos.y,&pos.z);
				//__PRINTINFO("SOLO_TOWER_MAP_INIT: format = %s , roomoffset=%s , i = %d, roomnum = %d, x=%f, y=%f, z=%f \n", format , roomoffset.c_str(), i, roomnum, pos.x, pos.y, pos.z );
