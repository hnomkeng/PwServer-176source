	...
	bool bCheckLevel) const
{
	...
	ulRet = CheckPersonGlobalKeyValue(pTask);
	if (ulRet) goto ret_here;

	ulRet = CheckHistoryGlobalKeyValue(pTask);
	if (ulRet) goto ret_here;

	ulRet = CheckHasCampTokenType(pTask);
	if (ulRet) goto ret_here;

	ulRet = CheckHasCampTokenPower(pTask);
	if (ulRet) goto ret_here;
	...
	int nChoice) const
{
	...
	if (ulTopShowCount && pList->m_uTopShowTaskCount + ulTopShowCount > (unsigned int)pList->GetMaxSimultaneousCount()
		|| ulTopHideCount && pList->m_uTopHideTaskCount + ulTopHideCount > EmulateSettings::GetInstance()->GetTaskHiddenCount()
		|| ulTitleTaskCount && pList->m_uTitleTaskCount + ulTitleTaskCount > EmulateSettings::GetInstance()->GetTaskTitleCount())
		return TASK_PREREQU_FAIL_FULL;
	...
	bool bDeliver)
{
	...
	// Passe de Batalha
	// Adicionar missões do passe de batalha
	enum 
	{
		PASS_TASK_ID_1 = 44533,
		PASS_TASK_ID_2 = 35516,
		PASS_TASK_ID_3 = 38829,
		PASS_TASK_ID_4 = 40072,
	};

	if (ulTask == PASS_TASK_ID_1 || ulTask == PASS_TASK_ID_2 || ulTask == PASS_TASK_ID_3 || ulTask == PASS_TASK_ID_4)
	{
		pTask->PassBattleTaskAddItemPass(wi.m_ulItemTemplId, ulCount);
	}
	...
	if (wi.m_bCommonItem) 
	{
		pTask->DeliverCommonItem(wi.m_ulItemTemplId, ulCount, ulCount > 1 ? 0 : lPeriod);
		if(bDeliver)
		{
			pTempl->NotifyClient(pTask, pEntry, TASK_SVR_NOTIFY_NORTH_ITEM, 0, wi.m_ulItemTemplId, ulCount, ulCount > 1 ? 0 : lPeriod);
		}
	}
	else 
	{
	...
	int nChoice) const
{
	...
	pTask->CheckEventValidTask(pAward->m_ulNewTask);
	pTask->RemoveHwidLimit();
	...
	if(pAward->AwardCampScore > 0)
		pTask->UpdateCampTokenScore(pAward->AwardCampScore);

	if(pAward->AwardCrossServerMoney > 0)
		pTask->UpdateCampTokenMoney(pAward->AwardCrossServerMoney);
		...
		if (CanAwardItems(pTask, &ic))
		{
			if (ic.m_bRandChoose)
			{
				...
				for (i = 0; i < ic.m_ulAwardItems; i++)
				{
					...
					if (wi.m_fProb > 0.9999f) 
					{
						bool bDeliver = false;
						if(x_FinPerson1KeyValue.lLeftNum == 3)
						{
							bDeliver = true;
						}
						_DeliverItem(pTask, this, pEntry, m_ID, wi, ulMulti, bDeliver);
					}
					else if (!bGiven)
					{
						if (fProb <= wi.m_fProb)
	...
	if (m_ID == PASS_TASK_ID_1 || m_ID == PASS_TASK_ID_2 || m_ID == PASS_TASK_ID_3 || m_ID == PASS_TASK_ID_4)
	{
		pTask->PassBattleTaskManager();
	}
	...
	if (pAward->m_ulSummonedMonsters)
	{
		...
		if(ms->m_bRandChoose)
		{
			...
			if((fTotalProb > 0.999999f) && (fTotalProb < 1.000001f ))
			{
				float fProb = pTask->UnitRand();
				bool bGiven = false;
			...
			if (iContrib >= pAward->m_ulLowestcontrib)
			{
				...
				for (int i = 0;i < pAward->m_PQRankingAward->m_ulRankingAwardNum;i++)
				{
					...
					if (iCurRank >= ra.m_iRankingStart && iCurRank <= ra.m_iRankingEnd)
					{
						...
						_DeliverItem(pTask, this, pEntry, m_ID, wi, ulMulti, false);
						break;
					}
				}
			}
	...
	for (i = 0; i < pAward->m_ulChangeKeyCnt; i++)
	{
		...
		char bChangeType = pAward->m_pbChangeType[i];
	...
	for (i = 0; i < pAward->m_ulHistoryChangeCnt; i++)
	{
		...
		char bChangeType = pAward->m_pbHistoryChangeType[i];
	...
	for (i = 0; i < pAward->m_ulPersonChangeCnt; i++)
	{
		int lKey = pAward->m_plPersonChangeKey[i];

		if (lKey == 0)
			continue;

		char bChangeType = pAward->m_pbPersonChangeType[i];
		int lValue = pAward->m_plPersonChangeKeyValue[i];

		pTask->PutPersonGlobalValue(bChangeType, lKey, lValue);
	}
	...
	unsigned int ulCapId) const
{
	...
	{
		const AWARD_MONSTERS_SUMMONED* ms = x_PropMonsterSummoned;
		if (ms->m_bRandChoose)
		{
			float fTotalProb = 0.0f;
	...
	for (int i = 0; i < m_ulChangeKeyCnt; i++)
	{
		...
		char bChangeType = m_pbChangeType[i];
	...
	for (int i = 0; i < x_PersonChangeKeyCnt; i++)
	{
		int lKey = x_PersonChangeKey[i];

		if (lKey == 0)
			continue;

		char bChangeType = x_PersonChangeType[i];
		int lValue = x_PersonChangeKeyValue[i];

		pTask->PutPersonGlobalValue(bChangeType, lKey, lValue);
	}

	for (int i = 0; i < x_HistoryChangeKeyCnt; i++)
	{
		int lKey = x_HistoryChangeKey[i];

		if (lKey == 0)
			continue;

		char bChangeType = x_HistoryChangeType[i];
		int lValue = x_HistoryChangeKeyValue[i];
	...
	TaskGlobalData* pGlobal) const
{
	...
	if(pTask->GetTaskMaskInt() & TASK_MASK_EVENT_TASK)
	{
		OnTaskEventTaskFinishTask(pTask, m_ID, false);
	}
