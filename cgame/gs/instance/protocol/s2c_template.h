...
namespace  S2C
{
	namespace CMD
	{
		...
		{
			template <typename WRAPPER>
			inline static WRAPPER & From(WRAPPER & wrapper,int hp, struct basic_prop & bp, struct extend_prop & ep, char combat_state, int targetid, int glyph_pvp_attr, int level_ranking)
			{
				return wrapper <<  bp.level << combat_state << (unsigned char)bp.sec.level << hp << ep.max_hp << bp.mp << ep.max_mp << targetid << glyph_pvp_attr << level_ranking;
			}
		...
		{
			template <typename WRAPPER>
			inline static WRAPPER & From(WRAPPER & wrapper,const XID & id,int hp,struct basic_prop & bp,struct extend_prop & ep,char combat_state, int targetid, int glyph_pvp_attr, int level_ranking)
			{
				Make<single_data_header>::From(wrapper,PLAYER_INFO_00);
				wrapper << id.id;
				return Make<INFO::player_info_00>::From(wrapper, hp, bp,ep,combat_state,targetid, glyph_pvp_attr, level_ranking);
			}
			template <typename WRAPPER>
		...
		{
			template <typename WRAPPER>
			inline static WRAPPER & From(WRAPPER & wrapper,struct basic_prop & bp, struct extend_prop & ep,char combat_state, char cheat_mode, int level_ranking)
			{
				int Unk = 0;
				...
				if(cheat_mode)
				{
					return wrapper << bp.level << combat_state << (unsigned char) bp.sec.level 
						<< 1 << ep.max_hp << 1 << ep.max_mp << bp.exp << bp.skill_point << Unk << ep.max_ap << level_ranking;
				}
				else
				{
					return wrapper << bp.level << combat_state << (unsigned char) bp.sec.level 
						<< bp.hp << ep.max_hp << bp.mp << ep.max_mp << bp.exp << bp.skill_point << bp.ap << ep.max_ap << level_ranking;
				}
			}
		};
		...
		struct Make<CMD::self_info_00>
		{
			...
			inline static WRAPPER & From(WRAPPER & wrapper,struct basic_prop & bp, struct extend_prop & ep,char combat_state, char cheat_mode, int level_ranking)
			{
				...
				return Make<INFO::self_info_00>::From(wrapper, bp,ep,combat_state,cheat_mode,level_ranking);
			}
		};
		...
		struct Make<CMD::send_equipment_info>
		{
			...
			inline static WRAPPER & From(WRAPPER & wrapper,gplayer* pPlayer,uint64_t mask,OCTETS &data, void * codex_fashion, unsigned int codex_size)
			{
				Make<single_data_header>::From(wrapper,EQUIPMENT_DATA);
				wrapper << (unsigned short)pPlayer->crc << pPlayer->ID.id << mask;	
				wrapper.push_back(codex_fashion, codex_size);	
		...
		template <>
		struct Make<CMD::stop_policy_info>
		{
			template <typename WRAPPER>
			inline static WRAPPER & From(WRAPPER & wrapper)
			{
				return Make<single_data_header>::From(wrapper,STOP_POLICY_INFO);
			}			
		};

		template <>
		struct Make<CMD::new_arena_team_created>
		{
			template <typename WRAPPER>
			inline static WRAPPER & From(WRAPPER & wrapper)
			{
				return Make<single_data_header>::From(wrapper,NEW_ARENA_TEAM_CREATED);
			}			
		};

		template <>
		struct Make<CMD::new_arena_send_info_queue>
		{
			template <typename WRAPPER>
			inline static WRAPPER & From(WRAPPER & wrapper,int mode)
			{
				Make<single_data_header>::From(wrapper,NEW_ARENA_TEAM_QUEUE);
				return wrapper << mode;
			}
		};
...
// -- cgame recv new 155 - 162 -- //
		...
		{
			template <typename WRAPPER>
			inline static WRAPPER& From(WRAPPER& wrapper, unsigned int team_type, int red_count_alive, int blue_count_alive, int reserved1, int reserved2, const char * redname, const char * bluename)
			{
				Make<single_data_header>::From(wrapper, ARENA_PLAYER_FACTION_INFO);
				wrapper << team_type << red_count_alive << blue_count_alive << reserved1 << reserved2;
				wrapper.push_back(redname, 36);
				wrapper.push_back(bluename, 36);
				return wrapper;
			}					
		};
		...
		{
			template <typename WRAPPER>
			inline static WRAPPER& From(WRAPPER& wrapper, unsigned int time_rest, long long blue_damage, long long red_damage, int team_count_members_red, int team_count_members_blue, int * members_red, int * members_blue)
			{
				Make<single_data_header>::From(wrapper, ARENA_BATTLE_GROUP_INFO);
				wrapper << time_rest << blue_damage << red_damage << team_count_members_red << team_count_members_blue;
		...
		{
			template<typename WRAPPER>
			inline static WRAPPER & From( WRAPPER & wrapper, int rest1, int rest2)
			{
				Make<single_data_header>::From(wrapper,GET_FINISH_NOTIFY);
				return wrapper << rest1 << rest2;
			}
		};
		...
		template<>
		struct Make<CMD::question_task>
		{
			template<typename WRAPPER>
			inline static WRAPPER & From( WRAPPER & wrapper, int mode,  int progress, int question, int correct, int time)
			{
				Make<single_data_header>::From(wrapper,QUESTION_TASK);
				return wrapper << mode << progress << question << correct << time;
			}
		};

		template<>
		struct Make<CMD::qiling_rename_character>
		{
			template<typename WRAPPER>
			inline static WRAPPER & From( WRAPPER & wrapper, int mode,  int roleid, char res1, char res2, char res3, char res4 )
			{
				Make<single_data_header>::From(wrapper,QILING_RENAME_CHARACTER);
				return wrapper << mode << roleid << res1 << res2 << res3 << res4;
			}
		};
		...
		struct Make<CMD::cgame_recv_517>
		{
			...
			inline static WRAPPER & From( WRAPPER & wrapper )
			{
				Make<single_data_header>::From(wrapper,RANDOM_MALL_SHOPPING_REBATE);
				return wrapper;
			}
		};
		...
		struct Make<CMD::codex_pet_consum_info>
		{
			...
			inline static WRAPPER& From(WRAPPER& wrapper, int pet_egg_id, int res, short res2, int mount_points)
			{
				...
				wrapper << pet_egg_id << res << res2 << mount_points;
				return wrapper;
			}				
		};
		...
		struct Make<CMD::codex_fly_consum_info>
		{
			...
			inline static WRAPPER& From(WRAPPER& wrapper, int fly_id, char res, int speed, int fly_points)
			{
				...
				wrapper << fly_id << res << speed << fly_points;
				return wrapper;
			}				
		};
...
// -- cgame recv new 162 - 172 -- //
		...
		{
			template <typename WRAPPER>
			inline static WRAPPER& From(WRAPPER& wrapper, unsigned int count, int reserve, unsigned int * values)
			{				
				Make<single_data_header>::From(wrapper, KID_CELESTIAL_INFO);
				kid_celestial_info::KID_STRUCT * it = (kid_celestial_info::KID_STRUCT *)values;
				wrapper << reserve << count;
				for(unsigned int i = 0; i < count; i++)
				{
					wrapper << it[i].level << it[i].rank << it[i].exp << it[i].idx << 
					it[i].atk << it[i].atk_mag << it[i].def << it[i].def_mag[0] << 
					it[i].def_mag[1] << it[i].def_mag[2] << it[i].def_mag[3] << it[i].def_mag[4] <<
					it[i].hp << it[i].crit;
				}
		...
		// Novas habilidades
		template<>
        struct Make<CMD::monkey_wall_info>
		{
            template <typename WRAPPER>
                inline static WRAPPER & From(WRAPPER & wrapper, const A3DVECTOR &newpos, float max_pos, int effetct_id, unsigned int roleid, int unk1)
                {
                    Make<single_data_header>::From(wrapper, MONKEY_WALL_INFO);    
                    return wrapper << newpos << max_pos << effetct_id << roleid << unk1;
                }
        };

		template<>
        struct Make<CMD::monkey_ultimate_info>
		{
            template <typename WRAPPER>
                inline static WRAPPER & From(WRAPPER & wrapper, unsigned int roleid, char enabled)
                {
                    Make<single_data_header>::From(wrapper, MONKEY_ULTIMATE_INFO);    
                    return wrapper << roleid << enabled;
                }
        };

		// Novo Ranking
		template <>
        struct Make<CMD::ranking_update_info>
        {
            template <typename WRAPPER>
            inline static WRAPPER& From(WRAPPER& wrapper, short level, unsigned int exp, short count_kills, short count_deads, unsigned int exp_max, char lock_unlock)
            {
                Make<single_data_header>::From(wrapper, RANKING_UPDATE_INFO);
                wrapper << level << exp << count_kills << count_deads << exp_max << lock_unlock;				
                return wrapper;
            }
        };
		
		template <>
        struct Make<CMD::ranking_storage_info>
        {
            template <typename WRAPPER>
            inline static WRAPPER& From(WRAPPER& wrapper, int count_ranking, unsigned int size, const void * ranking)
			{
				Make<single_data_header>::From(wrapper, RANKING_STORAGE_INFO);
				wrapper << count_ranking;
				wrapper.push_back(ranking, size);	
				return wrapper;
			}
        };		
		...
		template <>
        struct Make<CMD::newhistory_system_info>
        {
            template <typename WRAPPER>
            inline static WRAPPER& From(WRAPPER& wrapper, char enabled_systeam, unsigned int player_level, unsigned int player_realm_level, char enabled_buff, char * enabled_level, 
			char * reward_level, char * enabled_realm_level, char * reward_realm_level)
			{
				Make<single_data_header>::From(wrapper, NEWHISTORY_SYSTEM_INFO);
				wrapper << enabled_systeam << player_level << player_realm_level << enabled_buff;

				for (int i = 0; i < 3; i++) 
					wrapper << enabled_level[i];

				for (int i = 0; i < 3; i++) 
					wrapper << reward_level[i];

				for (int i = 0; i < 3; i++) 
					wrapper << enabled_realm_level[i];

				for (int i = 0; i < 5; i++) 
					wrapper << reward_realm_level[i];
		...
		template <>
        struct Make<CMD::bossrush_notify_info>
        {
            template <typename WRAPPER>
            inline static WRAPPER& From(WRAPPER& wrapper, char enable)
            {
                Make<single_data_header>::From(wrapper, BOSSRUSH_NOTIFY_INFO);
                wrapper << enable;			
                return wrapper;
            }
        };
		...
		template <>
        struct Make<CMD::cash_reward_info>
        {
            template <typename WRAPPER>
            inline static WRAPPER& From(WRAPPER& wrapper, unsigned int lastedtime, unsigned int lastedgold, unsigned int nexttime, unsigned int nextgold)
			{
				Make<single_data_header>::From(wrapper, CASH_REWARD_INFO);
				wrapper << lastedtime << lastedgold << nexttime << nextgold;			
				return wrapper;
			}          
        };		

		template <>
        struct Make<CMD::pass_battle_info>
        {
            template <typename WRAPPER>
            inline static WRAPPER& From(WRAPPER& wrapper, int * ticket_count)
			{
				Make<single_data_header>::From(wrapper, PASS_BATTLE_INFO);

				for (int i = 0; i < 3; i++) 
					wrapper << ticket_count[i];
		...
		template <>
        struct Make<CMD::pass_battle_reward>
        {
            template <typename WRAPPER>
            inline static WRAPPER& From(WRAPPER& wrapper, char enabled, int * ticket_count)
			{
				Make<single_data_header>::From(wrapper, PASS_BATTLE_REWARD);
				wrapper << enabled;
		...
		template <>
        struct Make<CMD::new_arena_team_info>
        {
            template <typename WRAPPER>
            inline static WRAPPER& From(WRAPPER& wrapper, unsigned int size, const void * members)
			{
				Make<single_data_header>::From(wrapper, NEW_ARENA_TEAM_INFO);
				wrapper.push_back(members, size);	
				return wrapper;
			}
        };

		template <>
        struct Make<CMD::new_arena_team_ranking>
        {
            template <typename WRAPPER>
            inline static WRAPPER& From(WRAPPER& wrapper, int team_count, unsigned int size, const void * teams)
			{
				Make<single_data_header>::From(wrapper, NEW_ARENA_TEAM_RANKING);
				wrapper << team_count;
				wrapper.push_back(teams, size);	
				return wrapper;
			}
        };
