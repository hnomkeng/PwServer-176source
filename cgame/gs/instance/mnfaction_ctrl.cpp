...
int mnfaction_ctrl::GatherResourseTower(RESOURSE_TOWER & resouce_tower_map, int mine_tid, gplayer *pPlayer)
{
	...
	else
	{
		...
		{
			if(resourse_tower._state == STATE_ATTACKER_GATHER || resourse_tower._state == STATE_ATTACKER)//���ڲɼ������
			{
				return 0;
			}
			else
...
int mnfaction_ctrl::GatherNeutralTower(SWITCH_TOWER & neutral_tower_map, int mine_tid, gplayer* pPlayer)
{
	...
	if(switch_tower._owner_faction == 0)//����������δ��ռ��
	{
		if(pPlayer->IsBattleOffense())
		{
...
void mnfaction_ctrl::CheckResourceTowerState(RESOURSE_TOWER & resourse_tower_map, world * pPlane)
{
	...
	for(; it != resourse_tower_map.end(); ++it)
	{
		...
		else if(resourse_tower._state == STATE_ATTACKER_GATHER)
		{
			...
			{
				resourse_tower._state            = STATE_ATTACKER;//���״̬
				resourse_tower._gather_roleid    = 0;
				resourse_tower._gather_faction   = 0;
...
void mnfaction_ctrl::ResourceTowerDestroy(int faction, world *pPlane)//͸���ָ���ȫ�ֱ������ٽ�����Ѫ�� 
{
	int num = 0;
	if(faction & FACTION_BATTLEOFFENSE)
	{
...
void mnfaction_ctrl::TransmitResurrectPosPlayer(world * pPlane)//ÿ30S�Ѹ����(С����)�е���Ҵ��ͻ��Լ����Ļ���
{
	A3DVECTOR pos(0,0,0);
	MSG msg;
	BuildMessage(msg,GM_MSG_LONGJUMP,XID(GM_TYPE_PLAYER,0),XID(0,0),pos,world_manager::GetWorldTag(),0,&_attacker_incoming_pos,sizeof(_attacker_incoming_pos));
	pPlane -> BroadcastLocalSphereMessage(msg, _resurrect_pos[0], _resurrect_pos_range);//���͹�����������
	
	BuildMessage(msg,GM_MSG_LONGJUMP,XID(GM_TYPE_PLAYER,0),XID(0,0),pos,world_manager::GetWorldTag(),0,&_defender_incoming_pos,sizeof(_defender_incoming_pos));
	pPlane -> BroadcastLocalSphereMessage(msg, _resurrect_pos[1], _resurrect_pos_range);//�����ط���������
}
