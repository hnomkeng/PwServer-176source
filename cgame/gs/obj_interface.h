...
public:
	...
	void ClearTwoRandomSpecFilters(int mask);
	void ClearOneRandomSpecFilter(int mask);
...
public:
	...
	void SendAttack2(const XID & target, attack_msg & attack);
	//����
	...
	void SendClientInterfaceEffect(int mode, int scene_id, int scene_value);
	...
	// Novas Habilidades
	void SendClientWallInfo(const A3DVECTOR &newpos, float max_pos, int effetct_id, unsigned int roleid, int unk1);
	void SendClientUltimateInfo(unsigned int roleid, char enable);
	void SendClientQilingRename(int mode, int roleid, char res1, char res2, char res3, char res4 ) ;
...
public:
	...
	int GetTeamVisibleStateParam(unsigned short state);
...
public:
	...
	void IncMaxDefense(float inc);
	void DecMaxDefense(float inc);
	float GetMaxDefense();
	...
	void SetNewShieldPassBattle(char b);
	void SetNewShieldCrystal(char b);
	void SetNewBlockTower(char b);
	void SetMonkyPassiveReduce(int inc, bool add);
	void SetMonkyBuffReduce(int inc, bool add);
	...
	void ImpairSoulPowerEN(int val);
	void ImpairSoulPowerRatio(float val);
	void EnhanceSoulPower(int val);
	void EnhanceSoulPowerEN(int val);
	void EnhanceSoulPowerRatio(float val);
	void ClearSoulPower1();
	void ClearSoulPower2();
	void SoulPowerRatio();
...
//��ȴ
	...
	int CalcPhysicDamage(int raw_damage, int attacker_level, int highest_level);
	int CalcMagicDamage(int dmg_class, int raw_damage, int attacker_level, int highest_level);
...
public:
	...
	//����������������npc��id(ע���Ǹ�ֵ)����ʧ�ܷ���-1
	int CreateMinors(const A3DVECTOR & pos ,const minor_param & p, int roleid=0);//��ָ��λ�ô���С��
	int CreateMinors(const minor_param & p,float radius = 6.0f, int roleid=0);	//�ڸ��������λ�ô���С��?
	int CreateMinors(const minor_param & p, const XID & target, float radius = 6.0f, int roleid=0);	//��Ŀ�긽�������λ�ô���С��?
	//���º����ɹ�����0��ʧ�ܷ���-1
	...
	void SetMaxReceivedDamagePerHit(int val);
...
public:
	...
	void EnhanceChargeMerc(unsigned int val); //Adiciona
	void ImpairChargeMerc(unsigned int val); //Remove
	void EnhanceChargeElseMerc(int roleid, unsigned int val); //Adiciona
	void ImpairChargeElseMerc(int roleid, unsigned int val); //Remove
	unsigned int GetCountChargeMerc();
	...
	void SummonMine(int mine_id, int count, const XID& target, int target_distance, int remain_time=0);

	void FactionCongregateRequest(int reply_level_req, int reply_sec_level_req, int reply_reincarnation_times_req);

	void ConvertPercDamageFtoDefResist(int type, float ratio);

	//void ClearAllSTOPMode();
	
	int GetCarrierID();

	void IncGenesisLevel(int val);
	void DecGenesisLevel(int val);
	int GetGenesisLevel();
	void EnhanceGenesisLevel(int value);
	void ImpairGenesisLevel(int value);

	int GetNPCID();

	void EnhanceHurtReduceRate(float val);
	void ImpairHurtReduceRate(float val);

	void SetSkillToTargetCursedShooter(const XID & target);
