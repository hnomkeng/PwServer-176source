...
/*	class ai_object
	Represents an object interface with ai behavior
	Various data required for various judgments and operations on all objects,
	All need to be completed according to this interface
...
public:
	...
	virtual int GetMP() = 0;
	virtual void BeHurt(int hp) = 0; //Reduce blood value
	...
	virtual void RegroupAggro() = 0;			//Set the hatred of all enemies to the same value
	virtual void SwapAggro(int index1,int index2) = 0;	//Swap two places in the hatred table
	virtual void BeTaunted(const XID & target) = 0;		//put someone first on the hate list
	virtual void FadeTarget(const XID & target) = 0;	//put someone last on the hate list
	virtual void AggroFade() = 0;				//Hatred Decay 50%
	...
	virtual bool IsInIdleHeartbeat() = 0;	//Is there no one around
...
public:
	...
	virtual void SummonMonster(int mod_id, int count, const XID& target, int target_distance, int remain_time,char die_with_who, int path_id, int roleid) = 0;	
...
public:
	...
	enum
	{
		//The main definitions of these enumerations are in actobject.h
		SEAL_MODE_NULL 		= 0x00,
		SEAL_MODE_ROOT 		= 0x01,
...
/**
	class ai_task
	The class ai_task describes the behavior pattern of an ai,
	The operation of this class will perform various behaviors corresponding to the mode
*/
...
class ai_magic_task : public ai_target_task
{
	...
	float _skill_range;
	int _ko_count;
	int _skill_type;	//0 blessing 1 curse 2 attack
	enum
	{
		STATE_START,
		STATE_TRACE,
...
class ai_follow_master : public ai_target_task		//This follow operation is specifically for following the captain
{
...
class ai_follow_target : public ai_target_task	//Follow action for siege engines 
{
	ai_follow_target();
public:
...
class ai_pet_follow_master : public ai_target_task		//This follow operation is for pets to follow their owners
{
...
/**
*	This type of task will be removed in the future, it is only a temporary solution now
*/
...
struct ai_param
{
	...
	int path_id;		 //Patrol route ID, if less than or equal to 0, then
	int path_type;		 //Patrol type 0 means no loop, 1 means: return to the original path (loop)?? It means that the end point is reached and the start point is cycled like this
	char patrol_mode;	 //Whether to parade randomly
	char speed_flag;	 //Whether high-speed patrol
	char no_auto_fight;		//Cannot enter combat state
	float max_move_range;	//Maximum range of movement
...
protected:
	...
	int _life;		//Lifetime is calculated according to time
	struct __SKILLS_T
	{
		int count;
		int skill[8];
...
DECLARE_SUBSTANCE(ai_policy);
	...
	int GetPrimarySkill(int & skill, int & level)
	{
		...
		{
			case 0:
			//First use prioritizes blessings, then curses
			...
			case 2:
			//In the future, 80% will consider ordinary skills, 10% will consider curses, and 10% will consider blessings
				{
					_policy_flag = 2; 
	...
	{
		//return to normal state no action required
		_idle_mode = isIdle;
		if(isIdle)
		{
			//enter wait state
		...
		{
			//Return to normal state and try to make an attack judgment
			DeterminePolicy();
		}
	}
	...
	void SetSealMode(int seal_flag)
	{
		...
		if(seal_flag)	//The fixed body is also handled in this way
		{
			//The current task should be cleared and regenerated?
			ClearTask();
		}
	}

public:
	inline bool InCombat() { return _in_combat_mode;} //Whether the detection of the policy layer is in the combat state
	inline bool IsFastRegen() { return _fast_regen;} //Is it possible to recover blood quickly
...
public:
	virtual void OnAggro();			//Hate Change Event
	virtual void OnHeartbeat();		//heartbeat
	virtual void Reborn()
	{
		Clear();
	...
	virtual void ChangePetAttackType(int) {}
	virtual void ChangePetForceAttack(char) {}
	...
	void CastSkill(int skill_id, int skill_level);

	void AddCastSkill(int skill,int level ,  const XID & who)
	{
		ai_task * pTask = NULL;
		pTask = new ai_skill_task(who,skill,level);
		pTask->Init(_self,this);
		AddTask(pTask);
	}
public:
...
DECLARE_SUBSTANCE(group_master_policy);
	//The current logic is no different from aipolicy
