#include "world.h"
#include "player_pvp_camp.h"
#include "clstab.h"

DEFINE_SUBSTANCE(gplayer_pvp_camp_imp,gplayer_imp,CLS_CAMPTOKEN_PLAYER_IMP)

int
gplayer_pvp_camp_imp::MessageHandler(world * pPlane ,const MSG & msg)
{
	switch(msg.message)
	{
		case GM_MSG_ATTACK:
		{
				...
				if(attacker.IsPlayerClass())
				{
					...
					int windex1;
					gplayer *gPlayer1 = world_manager::GetInstance()->FindPlayer(attacker.id, windex1);
					if (!gPlayer1 || !gPlayer1->imp) return 0;

					gplayer_imp *pImp1 = (gplayer_imp *)gPlayer1->imp;
					if (!pImp1) return 0;

					int windex2;
					gplayer *gPlayer2 = world_manager::GetInstance()->FindPlayer(_parent->ID.id, windex2);
					if (!gPlayer2 || !gPlayer2->imp) return 0;

					gplayer_imp *pImp2 = (gplayer_imp *)gPlayer2->imp;
					if (!pImp2) return 0;

					if(pImp1->GetAssociation()->GetAssociation()->camp_type == pImp2->GetAssociation()->GetAssociation()->camp_type)
					{
						return 0;
					}
...
int 
gplayer_pvp_camp_imp::ZombieMessageHandler(world * pPlane ,const MSG & msg)
{
	switch(msg.message)
	{
...
void
gplayer_pvp_camp_imp::FillAttackMsg(const XID & target,attack_msg & attack,int dec_arrow)
{
...
void 
gplayer_pvp_camp_imp::OnDeath(const XID & lastattack,bool is_pariah, char attacker_mode, int taskdead)
{
	gplayer_imp::OnDeath(lastattack, is_pariah, attacker_mode, taskdead);
}

void
gplayer_pvp_camp_imp::OnHeartbeat(unsigned int tick)
{
	if(_parent->IsZombie())
	{
...
void
gplayer_pvp_camp_imp::FillEnchantMsg(const XID & target,enchant_msg & enchant)
{
...
void 
gplayer_pvp_camp_imp::PlayerEnterWorld()
{	
...
void 
gplayer_pvp_camp_imp::PlayerEnterServer(int source_tag)
{
...
void 
gplayer_pvp_camp_imp::PlayerLeaveServer()
{
...
gplayer_pvp_camp_imp::GetPetAttackHook()
{
	return gplayer_pvp_camp_imp::__GetPetAttackHook;
	
}

gactive_imp::enchant_judge 
gplayer_pvp_camp_imp::GetPetEnchantHook()
{
	return gplayer_pvp_camp_imp::__GetPetEnchantHook;
}

gactive_imp::attack_fill 
gplayer_pvp_camp_imp::GetPetAttackFill()
{
	return __GetPetAttackFill;
}

gactive_imp::enchant_fill 
gplayer_pvp_camp_imp::GetPetEnchantFill()
{
...
bool 
gplayer_pvp_camp_imp::__GetPetAttackHook(gactive_imp * __this, const MSG & msg, attack_msg & amsg)
{
...
bool 
gplayer_pvp_camp_imp::__GetPetEnchantHook(gactive_imp * __this, const MSG & msg,enchant_msg & emsg)
{
...
void 
gplayer_pvp_camp_imp::__GetPetAttackFill(gactive_imp * __this, attack_msg & attack)
{
...
void 
gplayer_pvp_camp_imp::__GetPetEnchantFill(gactive_imp * __this, enchant_msg & enchant)
{
