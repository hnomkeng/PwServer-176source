...
void
player_bind::PlayerLinkRequest(gplayer_imp * pImp, const XID & target, int param_id)
{
	if(!pImp->IsPlayerFemale() && !param_id)
	{
	...
	//发出消息
	pImp->SetBindParam(param_id);
	pImp->SendTo<0>(GM_MSG_PLAYER_BIND_REQUEST,target,0,&param_id,sizeof(int) );

}

void
player_bind::PlayerLinkInvite(gplayer_imp * pImp, const XID & target, int param_id)
{
	...
	BIND_TARGET _msg;
	_msg.param_id = param_id;
	_msg.target = target;

	//发出消息
	pImp->SetBindParam(param_id);
	pImp->SendTo<0>(GM_MSG_PLAYER_BIND_INVITE,target,0,&param_id,sizeof(int));
}

void
player_bind::PlayerLinkReqReply(gplayer_imp * pImp, const XID & target, int param, int param_id)
{
	if(pImp->IsPlayerFemale() && !param_id)
	{
		pImp->_runner->error_message(S2C::ERR_INVALID_GENDER);
...
void
player_bind::PlayerLinkInvReply(gplayer_imp * pImp, const XID & target,int param, int param_id)
{
	if(!pImp->IsPlayerFemale() && !param_id)
	{
		pImp->_runner->error_message(S2C::ERR_INVALID_GENDER);
...
void 
player_bind::MsgRequest(gplayer_imp * pImp, const XID & target, const void * _vmsg, unsigned int len)
{
	if(len != sizeof(int)) return;
	int param_id = *(int*)_vmsg;
	
	//男
	if(pImp->IsPlayerFemale() && !param_id) return;
	...
	//通知消息给客户端
	pImp->SetBindParam(param_id);
	pImp->_runner->player_bind_request(target, param_id);
}

void 
player_bind::MsgInvite(gplayer_imp * pImp, const XID & target, const void * _vmsg, unsigned int len)
{
	if(len != sizeof(int)) return;
	int param_id = *(int*)_vmsg;
	...
	//通知消息给客户端
	pImp->SetBindParam(param_id);
	pImp->_runner->player_bind_invite(target,param_id);
