...
/*	static inline bool CopyTemplate(int cls, extend_prop & data)
	...
	*/
	...
	{
		return MAX_REALM_LEVEL-10; //Majestoso
	}
...
*/
	...
	static inline float GetDamageReduce(int defense, int attacker_level, int highest_level, float inc_max_defense)
	{
		float _defense_max = 0.95f;
		_defense_max += inc_max_defense;
		
		float def = defense;
		def = def / (def + 40.f * (attacker_level + highest_level) - 25.f);
		if(def > _defense_max) def = _defense_max;
		return def;
	}
    ...
    static inline int GetViabilityScore(gactive_imp* pImp, unsigned int& value)
    {
		...
		short highest_level = pImp->_highest_level;		
        if (level <= 0) return 0;
		if (highest_level <= 0) highest_level = 0;

		float _defense_max = 0.95;
		_defense_max += pImp->_inc_max_defense;
	
		float damage_reduce1 = pImp->_cur_prop.defense / (pImp->_cur_prop.defense + 40.0 * (level + highest_level) - 25.0);		
        if (damage_reduce1 > _defense_max) damage_reduce1 = _defense_max;

        float damage_reduce2 = pImp->_damage_reduce * 0.01;
        if (damage_reduce2 > _defense_max) damage_reduce2 = _defense_max;
		...
		float magic_damage_reduce1 = resistance / (resistance + 40.0 * (level + highest_level) - 25.0);
		
        if (magic_damage_reduce1 > _defense_max) magic_damage_reduce1 = _defense_max;

        float magic_damage_reduce2 = magic_damage_reduce * 0.01;
        if (magic_damage_reduce2 > _defense_max) magic_damage_reduce2 = _defense_max;
	...
	{
		static float prop[10] = {1.1f,0.0f,0.00,0.0f,0.f,0.f,0.f};
		return prop;
	}
