...
global_world_manager::HandleSwitchRequest(int link_id,int user_id, int localsid,int source, const instance_key & key)
{
	...
	//�����ȴ���Ϣ
	MSG msg;
	BuildMessage(msg,GM_MSG_SWITCH_GET,pPlayer->ID,XID(GM_TYPE_SERVER,world_manager::GetWorldIndex()),A3DVECTOR(0,0,0),GetWorldTag(),0, &key, sizeof(key));
...
void global_world_manager::OnMafiaPvPStatusNotice(int status,std::vector<int>& ctrl_list)
{
	...
	BuildMessage(msg,GM_MSG_MAFIA_PVP_STATUS,wid,wid,A3DVECTOR(0,0,0),status,0,&ctrl_list.front(),ctrl_list.size()*sizeof(int));
