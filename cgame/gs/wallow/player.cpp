...
//	__PRINTF("player %d heart beat\n", _parent->ID.id);
	...
	if (_kid_transformation)
	{
		...
		if(_kid_transformation_time <= 0)
		{
			...
			KidCelestialTransformation(1);
		}
	}
	
	if (!(_write_timer % 45)) // Verifica a cada 30 segundos
	{
		SetAnecdotePoints();
	}
...
{   
    ...
    } // Faz a busca do item e o ponto de producao
...
gplayer_imp::CarrierHeartBeat()
{
	...
	{
		EnterCarrier(_carrier.npcid, _carrier.pos, _carrier.reserved1);
	}
}
...
{
	if(LuaManager::GetInstance()->GetConfig()->activity_event_enable < 1 || !_newhistory.GetEnabledBuff())
	return;
...
{
	if(!_newhistory.GetEnabledBuff())
	{
		if(!IsExistTeamVisibleState(GNET::HSTATE_547)) return;
...
gplayer_imp::ProtocolCelestialMemorialLottery()
{
	...
	for(unsigned int i = 0; i < CelestialMemorialConfig::COUNT_LOTTERY_RED_BOX; i++)
	{
		...
		{
			lotterys[i].storage_rest_cnt[n] = (int)_celestial.GetCelestialUsePergProb(i);
		}
...
void 
gplayer_imp::KidCelestialInfoProtocol()
{
	...
	{
		celestial[i].level = _kid.GetCelestial(i)->level;
		celestial[i].rank = _kid.GetCelestial(i)->rank;
		celestial[i].exp = _kid.GetCelestial(i)->exp;
		celestial[i].idx = _kid.GetCelestial(i)->idx;

		//GLog::log(GLOG_ERR, "celestial[5]:: _kid.GetCelestial(i)->level = %d, _kid.GetCelestial(i)->rank = %d, _kid.GetCelestial(i)->exp = %d, _kid.GetCelestial(i)->idx = %d", _kid.GetCelestial(i)->level, _kid.GetCelestial(i)->rank, _kid.GetCelestial(i)->exp, _kid.GetCelestial(i)->idx);

		celestial[i].atk = 1000;
		celestial[i].atk_mag = 1100;
		celestial[i].def = 100;

		for (unsigned int y = 0; y < 5; y++)
		{
			celestial[i].def_mag[y] = 500;
		}

		celestial[i].hp = 15000;
		celestial[i].crit = 5;
	}

	//GLog::log(GLOG_ERR,"celestial[6]:: celestial[0].level = %d, celestial[0].rank = %d, celestial[0].exp = %d, celestial[0].idx = %d, celestial[0].atk = %d, celestial[0].atk_mag = %d, celestial[0].def = %d, celestial[0].hp = %d, celestial[0].crit = %d", celestial[0].level, celestial[0].rank, celestial[0].exp, celestial[0].idx, celestial[0].atk, celestial[0].atk_mag, celestial[0].def, celestial[0].hp, celestial[0].crit);
	
	_runner->kid_celestial_info(gplayer_kid::MAX_CELESTIAL, 0, (unsigned int*)celestial);
}
...
gplayer_imp::KidAwakeningNewDay2()
{
	...
	{	
		...
		_runner->kid_awakening_points(_kid.GetPointsAwakening());
	}
...
gplayer_imp::KidAwakeningCardRemoveInventory(char old_slot)
{
	...
	cash_cost = cash_cost + (ess->cost + course_level);
...
{
	if (_kid.GetAwakeningDayCount() == gplayer_kid::REQUIRED_DAYS_FOR_AWAKENING)
	{
		...
		{
			// Caso já exista um bebê celestial do mesmo tipo, ele faz upgrade do idx e entrega o item.
			if (_kid.GetCelestial(config2->kid_debri_type)->idx < idx_item)
			{
				_kid.SetCelestial(config2->kid_debri_type, _kid.GetCelestial(config2->kid_debri_type)->level, _kid.GetCelestial(config2->kid_debri_type)->rank, _kid.GetCelestial(config2->kid_debri_type)->exp, idx_item);
				item_get = true;
			}
		...
		{
			_kid.SetCelestial(config2->kid_debri_type, 1, config2->rahk, 0, idx_item);	
			KidCelestialDebrisLevelUp(config2->kid_debri_type, config2->kid_debri_exp);		
		}

		// Limpa todo o despertar
		_kid.ClearAwakening();
...
static const int debris_exp_required_next_level[] = { 9, 90, 900, 1000, 2000, 4000, 8000, 16000, 32000 };

int gplayer_imp::KidCelestialDebrisLevelUp(int idx, int exp)
{
    enum 
    {
        MAX_LEVEL = 10,
    ...
    int new_level = -1;
    int get_exp_now = _kid.GetCelestial(idx)->exp;
    int get_level_now = _kid.GetCelestial(idx)->rank;

	if(get_exp_now+exp < debris_exp_required_next_level[get_level_now])
	{
   	 	_kid.SetCelestial(idx, _kid.GetCelestial(idx)->level, get_level_now, get_exp_now+exp, _kid.GetCelestial(idx)->idx);
		new_level = get_level_now;

	} else
	{
		int i = 0;
		
		while(i < MAX_LEVEL || get_level_now < MAX_LEVEL)
		{
			if(get_exp_now+exp >= debris_exp_required_next_level[get_level_now])
			{
				get_exp_now -= (exp+debris_exp_required_next_level[get_level_now]);
				get_level_now++;
				
				if(get_exp_now < 0) get_exp_now = 0;
				if(get_level_now > MAX_LEVEL) get_level_now = MAX_LEVEL;	

				_kid.SetCelestial(idx, _kid.GetCelestial(idx)->level, get_level_now, get_exp_now, _kid.GetCelestial(idx)->idx);
...
bool 
gplayer_imp::KidCelestialUpgradeRank(int celestial_idx, int where, int inv_idx)
{
	item_list & trashbox = GetTrashInventory(where);

	if(inv_idx >= trashbox.Size()) 
		return false;

	item & item_box = trashbox[inv_idx];

	if(item_box.type == -1) 
		return false;

	DATA_TYPE dt;
	KID_DEBRIS_ESSENCE * ess;
	ess = (KID_DEBRIS_ESSENCE*) world_manager::GetDataMan().get_data_ptr(item_box.type, ID_SPACE_ESSENCE, dt);
	if(!ess || dt != DT_KID_DEBRIS_ESSENCE) return false;

	int stone_exp = (ess->swallow_exp * item_box.count);
	int new_level = KidCelestialDebrisLevelUp(celestial_idx, stone_exp);

	// Remove o(s) item(s)
	UpdateMallConsumptionDestroying(item_box.type, item_box.proc_type, item_box.count);
	GetTrashInventory(where).DecAmount(inv_idx, item_box.count);
	_runner->player_drop_item(where,inv_idx,item_box.type,item_box.count,S2C::DROP_TYPE_USE);
...
gplayer_imp::KidCelestialTransformation(int mode)
{
	...
	{		
		...
		_runner->kid_celestial_transformation(config2->unk1,_parent->ID.id,30,now);
	}		
	else
	{		
		_kid_transformation = false;
...
{	
	...
	if(ranking_level_killed <= 0)
	{
		lua->game__ChatMsg(0, killer_id, 13, "^ff8080O Alvo não possuí uma patente. Pontos de experiência não foram adicionados.", -1);
		return;
	}

	if (RankingManager::GetInstance()->EnableFreeKill())
	{
		int windex4;
		gplayer *gPlayer_Killer4 = world_manager::GetInstance()->FindPlayer(killer_id, windex4);
		if (gPlayer_Killer4)
		{
			gplayer_imp *pImp4 = (gplayer_imp *)gPlayer_Killer4->imp;
			if (pImp4)
			{
				if (pImp4->GetRanking()->CheckForFreeKill(killed_id))
				{
					int rem_now_exp = pImp4->GetRanking()->GetPlayerRanking()->exp;
					int rem_count_kills = pImp4->GetRanking()->GetPlayerRanking()->count_kills;
					int rem_count_deads = pImp4->GetRanking()->GetPlayerRanking()->count_deads;
					int rem_now_level = pImp4->GetRanking()->GetPlayerRanking()->level;

					rem_now_exp = rem_now_exp - (rem_now_exp * 0.15);
					rem_count_kills = rem_count_kills - 10;

					pImp4->GetRanking()->SetPlayerRanking(rem_now_level, rem_now_exp, rem_count_kills, rem_count_deads);
					lua->game__ChatMsg(0, killer_id, 13, "^ff8080Você foi pego pelo nosso sistema de IA, você eliminou muitas vezes o(s) mesmo(s) jogador(es) em um curto período de tempo, seus pontos de experiência e mortes foram reduzidos.", -1);
					pImp4->ProtocolRankingUpdate();

					pImp4->SendPlayerOtherData(0, _parent->ID.id);
					pImp4->SendPlayerOtherData(1, _parent->ID.id);
					return;
				}
			}
		}
...
{	
	...
	int elo_killer_exp_gain = RankingManager::GetInstance()->GetElos(killed_ranking_level-1)->elo_killer_exp_gain;
	int elo_exp_max = RankingManager::GetInstance()->GetElos(killed_ranking_level-1)->elo_exp_max;
...
gplayer_imp::SendPlayerDataRenderInfoCodex()
{
	...
	packet_wrapper h1(24);
	packet_wrapper h2(24);
	struct FASHION
	{
		int fashion_id;
		int fashion_color;
	...
	CMD::Make<CMD::codex_fashion_activate>::From(h2, _parent->ID.id, _codex.GetCodexActivateFashion()->mode_classic, h1.size(), h1.data());
	send_ls_msg(_subscibe_list.begin(), _subscibe_list.end(), h2.data(),h2.size());
	send_ls_msg(((gplayer_dispatcher*)_runner)->_header,h2);
}
...
gplayer_imp::SendPlayerDataRenderInfoPortatil()
{
	...
	CMD::Make<CMD::portatil_picture_info>::From(h2, _parent->ID.id, 0, 0, _glua.GetPortatilPictureActive(), 0);
	send_ls_msg(_subscibe_list.begin(), _subscibe_list.end(), h2.data(),h2.size());
	send_ls_msg(((gplayer_dispatcher*)_runner)->_header,h2);
}

void gplayer_imp::ClearRewardCash()
{
	if(!GetLua()->GetClearRewardGold())
	{
		_dividend_mall_info.ClearDividend();
		_runner->player_dividend(_dividend_mall_info.GetDividend());

		GetLua()->SetClearRewardGold(true);
	}
}
