...
void public_quest::SetContrib(int role_id, int value) 
{
	spin_autolock keeper(_lock);
	SCORE_MAP::iterator it = _score.find(role_id);
	if(it != _score.end())
...
void public_quest_manager::SetCurContrib(int task_id, int role_id, int value)
{
	public_quest * quest = GetPublicQuest(task_id);
	if(quest != NULL)
	quest->SetContrib(role_id, value);	
}
...
int public_quest::GetAllContrib()
{
	int score_total = 0;
	for(SCORE_MAP::iterator it = _score.begin(); it != _score.end(); ++it)
	{
...
int public_quest_manager::GetAllCurContrib(int task_id)
{
	public_quest * quest = GetPublicQuest(task_id);
	if(quest != NULL)
	{
		return quest->GetAllContrib();
	}
	return 0;	
}
