...
gmatter_money_imp::OnPickup(const XID & who,int team_id, bool is_team)
{
	...
	if(team_id > 0)
	{
		BuildMessage(msg,GM_MSG_PICKUP_TEAM_MONEY,XID(GM_TYPE_PLAYER,team_id),_parent->ID,
				_parent->pos,_money,0,&_drop_user,sizeof(_drop_user));
	}
	else
	{
		...
		case GM_MSG_GATHER_REQUEST:
			{
				...
				//��������
				{
					...
					MSG message;
					BuildMessage(message,GM_MSG_GATHER_REPLY,msg.source,_parent->ID,_parent->pos,0,0,&rpy,sizeof(rpy));
				...
				if(_broadcast_aggro)
				{
					...
					BuildMessage(message,GM_MSG_AGGRO_ALARM,XID(GM_TYPE_NPC,-1),msg.source,_parent->pos,0,0,&alarm,sizeof(alarm));
		...
		case GM_MSG_GATHER:
			{
				...
				if (it != eit)
				{
					...
					BuildMessage(message,GM_MSG_GATHER_RESULT,msg.source,_parent->ID,_parent->pos,
							_produce_id,0,&data,sizeof(data));
					_plane->PostLazyMessage(message);
